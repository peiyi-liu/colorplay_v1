-- New classrooms use a shorter, fixed 8-character Crockford Base32 code.
-- Existing 16-character hexadecimal codes remain valid and unchanged.
...
alter table public.classrooms
  ...
  check (
    join_code is null
    or join_code ~ '^[0-9A-HJKMNP-TV-Z]{4}-[0-9A-HJKMNP-TV-Z]{4}$'
    or join_code ~ '^[0-9A-F]{4}(-[0-9A-F]{4}){3}$'
  );

create function public.generate_short_classroom_join_code()
returns table (
...
begin
  ...
  select
    generated_code,
    substr(generated_code, 1, 4) || '-' || substr(generated_code, 5, 4);
end;
$$;

revoke all on function public.generate_short_classroom_join_code()
...
begin
  ...
  for generation_attempt in 1..5 loop
    select generated.normalized_code, generated.display_code
    into normalized_code, display_code
    from public.generate_short_classroom_join_code() as generated;
...
begin
  ...
  if normalized_code is null
    or normalized_code !~ '^([0-9A-HJKMNP-TV-Z]{8}|[0-9A-F]{16})$'
  then
...
comment on function public.generate_short_classroom_join_code() is
  'Internal generator for fixed 8-character classroom join codes.';
