-- Avoid PostgreSQL CURRENT_TIME keyword resolution inside SQL statements.
...
begin
  ...
  where (
    ...
    and observed_at - limiter.window_started_at >= window_duration;
  ...
  if identity_count >= identity_limit or ip_count >= ip_limit then
    retry_after_seconds := greatest(
      case when identity_count >= identity_limit then
        ceil(extract(epoch from identity_window + window_duration - observed_at))::integer
      else 0 end,
      case when ip_count >= ip_limit then
        ceil(extract(epoch from ip_window + window_duration - observed_at))::integer
      else 0 end,
      1
    );
    return jsonb_build_object(
