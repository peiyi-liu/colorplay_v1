...
export async function collectPlatformMonitoring({
  ...
  const lease = await rpc('svc_admin_monitor_begin_collection', {
  ...
  if (lease?.outcome === 'busy')
    return {
      outcome: 'busy',
      retryAfterSeconds:
        Number.isInteger(lease.retry_after_seconds) &&
        lease.retry_after_seconds >= 0
          ? lease.retry_after_seconds
          : 60,
    };
  if (lease?.outcome !== 'started')
    throw new Error('MONITOR_LEASE_RESPONSE_INVALID');
  try {
    ...
    await collect('release_proof', async () => {
      ...
      return {
        status:
          ...
          marker.revision === receipt.revision
            ? 'ok'
            : 'attention',
        revision: receipt.revision,
        evidence_run_id: receipt.evidence_run_id,
    ...
    await rpc('svc_admin_monitor_record_collection', {
      p_request_id: requestId,
      p_observations: results,
    });
    return {
      outcome: 'ok',
      results: results.map(({ signal, status }) => ({ signal, status })),
    };
  } catch (error) {
    try {
      await rpc('svc_admin_monitor_finish_collection', {
        p_request_id: requestId,
        p_succeeded: false,
      });
    } catch {
      // Keep the original collection error. The database lease has a hard expiry.
    }
    throw error;
  }
}
