...
select is(jsonb_array_length(:'admins_page2'::jsonb -> 'rows'),
  greatest((select count(*)::int from public.admin_security_identities) - 50, 0),
  'Admin list second page returns the exact remaining rows');
