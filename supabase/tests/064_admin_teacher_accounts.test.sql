...
begin
  for i in 1..51 loop
    ...
    values ('00000000-0000-0000-0000-000000000000', v_id,
      ...
      '', '', '', '');
    update public.profiles
       set role = 'teacher', login_account = 'teacher' || (200 + i)::text,
           full_name = '分頁教師 ' || lpad(i::text, 2, '0'),
           display_name = '分頁教師 ' || lpad(i::text, 2, '0'),
...
select set_config('pgtap.teacher_page_1',
  public.admin_list_teachers(null, '分頁教師', null)::text, true);
...
select is((select row ->> 'contact_email_masked'
  ...
  where row ->> 'login_account' = 'teacher201'), 'o****@example.test',
...
select is(jsonb_array_length((public.admin_list_teachers(
  null, 'teacher217', null) -> 'rows')), 1,
  'search matches the login account');
select is(jsonb_array_length((public.admin_list_teachers(
  null, '分頁教師 18', null) -> 'rows')), 1,
