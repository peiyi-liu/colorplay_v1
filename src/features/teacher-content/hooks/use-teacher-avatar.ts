...
export const teacherAvatarQueryKey = (actorId: string) =>
  ['teacher', actorId, 'avatar'] as const;
...
> {
  ...
  return useQuery({
    ...
    queryKey: teacherAvatarQueryKey(actorId),
...
export function useUploadTeacherAvatar(
  ...
  return useMutation({
    ...
    onSuccess: (signedUrl) => {
      queryClient.setQueryData(teacherAvatarQueryKey(actorId), signedUrl);
    },
    retry: false,
  });
}
