...
const REVIEW_READER_LONG_CONTENT = `# 色彩三要素

1. **色相（Hue）**：色彩的名稱，也是辨認不同顏色最直接的線索。
   - 先找出色相環上的位置。
   - 再比較鄰近或對比關係。
2. **明度（Value）**：描述色彩的深淺與亮暗；重要資訊需要足夠的明度差。
3. **彩度（Saturation）**：表示色彩的鮮豔或灰濁程度，用來建立主次與控制注意力。
4. **三要素的關係**：任何具體顏色都能同時用色相、明度和彩度描述。
5. **觀察練習**：找一張熟悉的海報，依序記錄主要色相、最亮與最暗的位置，以及彩度最高的焦點。

### （三）彩度

彩度（Chroma，簡稱C）定義：色彩的鮮豔度（Saturation）、純粹度、==飽和度==或==純色量==。無彩色中的==黑、灰、白色就只有明度，而沒有彩度==概念。

彩度最高、最鮮艷的色彩稱為：純色（Pure Color）。

> 判讀時要分開比較三個屬性，避免把色相、明度和彩度混為一談。
...
> 延伸閱讀：觀察色彩時，先記錄主要色相與它在色相環上的位置。
> 再比較畫面中最亮與最暗的區域，確認資訊是否有足夠對比。
> 接著標出彩度最高的焦點，觀察視線是否依照設計順序移動。
> 如果降低焦點彩度，閱讀順序可能變得平緩，主次也可能不再明確。
> 如果縮小明度差，即使色相不同，文字與背景仍可能難以辨識。
> 暖色常讓人聯想到陽光、火焰與活力，但感受仍會受到面積影響。
> 冷色常讓人聯想到天空、水與寧靜，也會因周圍色彩而改變。
> 相鄰色相通常產生柔和、連續的感受，適合建立穩定的視覺節奏。
> 相對色相容易形成明顯對比，使用時要同時控制面積與明度。
> 高彩度顏色醒目而有張力，適合少量使用在關鍵資訊或互動焦點。
> 低彩度顏色沉穩而柔和，可用來支撐背景與次要資訊。
> 降低彩度不等於降低明度，兩者必須分開觀察與記錄。
> 灰濁的淺色與灰濁的深色都可能存在，不能只用鮮豔程度判斷亮暗。
> 設計配色時可先決定主要色相，再以明度確保閱讀清楚。
> 最後用彩度控制氣氛與焦點，並回頭檢查整體層次是否一致。
> 完成記錄後，再用去色畫面檢查明度關係，避免只依賴色彩名稱。

==重點整理==：三者互相影響，共同構成我們實際看到的色彩樣貌。`;
