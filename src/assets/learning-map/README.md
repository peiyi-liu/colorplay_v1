...
| ------------------------------- | ---------: | ------: | -------------------------------------------------------------------------- |
| `continuous-world-desktop.webp` |   1672×941 | 336,794 | Night continuous-world terrain with six desktop landing anchors            |
| `continuous-world-mobile.webp`  |   941×1672 | 275,130 | Night continuous-world terrain with six mobile landing anchors             |
...
Total optimized image bytes: **1,306,088** (1.246 MiB). The previous forest-village base remains temporarily for historical CSS compatibility and can be removed only after the generated-board map review is approved.
...
- Continuous-world desktop and mobile terrain: `exec-c5dacdf4-7463-470e-993b-8682d6a16af7.png` and `exec-440277ce-fddf-41de-9b59-1ed31cae7d5b.png`. These environment-only assets use the owner-adopted board 05 as composition guidance, contain exactly six empty landing areas, and contain no buildings, labels, status, CTA, HUD, or authoritative data.
...
The continuous-world terrain assets were encoded from the built-in image-generation PNG outputs through Chromium Canvas WebP at quality 0.88 with image smoothing disabled. Source PNGs remain recoverable under `/private/tmp/colorplay-continuous-world-*-source.png` for the current local session and are not part of the repository.
