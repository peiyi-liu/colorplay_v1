...
## 2026-08-29 14:34 [Codex] — teacher01 混合班級碼載入修復，加入班級限流已發布 Staging

- 根因與相容修正：`teacher01` 同時擁有兩個舊 16 位班級碼與一個新 8 位碼，前端 `list_owned_classrooms` 回應 schema 仍只接受 16 位碼，導致任一新碼使整批班級解析失敗；`teacher02`／`teacher03` 沒有班級資料，所以未觸發。repository 的 owner list／create／rotate schema 已統一接受 `XXXX-XXXX` Crockford 與既有 `XXXX-XXXX-XXXX-XXXX`，未改寫任何現有班級或加入碼。
- 安全邊界：新增 `join-classroom` Edge Function 與 service-only resolver／join command；authenticated 的舊 `join_classroom` RPC execute 已撤銷，避免繞過。錯碼採 10 分鐘固定視窗：單一 identity 第 10 次起、共享 IP 第 100 次起回 `CLASSROOM_JOIN_RATE_LIMITED`＋`Retry-After`；只保存 HMAC IP 指紋，不保存原始 IP。`student-register` 亦走同一 resolver，前端註冊與加入表單均提供繁中安全訊息。
- 發布：產品 commit `205efd09bb69d650b963a96ac6ec00753e8480e5` 與 follow-up `f497653` 已 push。Staging `onkxnkzeixpezetkmocf` 套用 `20260829000200_classroom_join_rate_limits.sql` 與 `20260829000300_fix_classroom_join_rate_limit_clock.sql`；後者修正 hosted smoke 抓到的 PL/pgSQL `current_time` 與 PostgreSQL keyword 衝突。`join-classroom`／`student-register` 已部署；web Preview `dpl_9wB2mkmgsdQFqGJX2sn8HM5knPU9` 已 promotion 為 `dpl_fD6NQhe4chWx8RBx9L3BDWf7PbYr`，`staging.colorplayapp.com` READY，bundle 只含 Staging Supabase ref。
- Hosted 驗證：以 owner 授權的舊教師帳密登入成功，落在 `/teacher`；分析篩選與 `/teacher/classes` 均完整載入 `Fixture Classroom One`、`配老師專班`、`0829-test`，班級數 3、有效學生合計 7，16／16／8 位加入碼同頁成立，console 0 error／0 warning。另以固定 ID 臨時學生呼叫真實 Edge Function：前 9 次錯碼均 HTTP 400，第 10 次 HTTP 429；測試前 limiter 表為 0，完成後以 fail-closed 條件刪除臨時 Auth／profile／identity 與兩筆計數，二次查詢皆為 0。
- 本機與邊界：相關 Vitest 5 檔／54 tests、`pnpm typecheck`、全 repo lint、production build、scoped Prettier 與 diff check 通過；全 repo Prettier 仍有 33 個既存非本輪檔案不合格式，未擴張修改。未執行 `pnpm test:db`、Supabase reset、Production 操作，未修改 Phase 0／1 保護路徑。Supabase CLI migration 已成功套用；其事後 pg-delta catalog cache 因暫存 CA 路徑警告失敗，不影響 migration history／權限或 hosted smoke。
