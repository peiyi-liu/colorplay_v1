...
- 做了什麼：先完成 Claude Code 唯一一次 plan review 的 4 High／4 Medium／1 Low remediation，並回寫核准 spec：第 8.4 節更正全域 `prefers-reduced-motion` 與 podium fireworks 既有覆寫的事實；第 6 節收旂 too-small 只在 podium／cancelled 沿用既有 exit，進行中 phase 不新增離開路徑；Task 3 補齊四 viewport 控制項 `>=44×44px`；內容強制邊界保持 U1 零新 API／RPC／schema／query／mutation，真正 import／server guard 移交 2A 或 5F-F2 且不阻塞 U1。專用 worktree 為 `.worktrees/phase5f-u1-live-presenter-ui`，branch `phase5f/u1-live-presenter-ui`，base `e559a5c32d7c685fb950033f97405680292cb1e5`。
