...
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `admin_sessions`               | insert (new session on challenge success), update of `last_activity_at`/`last_totp_verified_at`, revoke (supersede on a second concurrent session) | Challenge and every subsequent authorized touch                                                                                                                                                                                                                  |
...
- Reveal any real personal field (`admin_reveal_field` against a live user's
  actual data) for any purpose other than the specific fixture-free
  verification the smoke is scoped to; a smoke run should prefer probing the
  reveal _denial_ paths (missing purpose, stale session) over a real reveal
  wherever the gate's evidence requirement allows it, and any real reveal it
  does perform must have its own genuine, auditable purpose string — never a
  placeholder like "smoke test".
- Deactivate, tombstone, or otherwise change the lifecycle state of any
  Admin identity other than the smoke run's own (and even then, only through
  the normal in-product paths — never by calling an OOB service function
  from a smoke script).
