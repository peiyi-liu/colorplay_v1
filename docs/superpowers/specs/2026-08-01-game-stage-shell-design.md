...
7. **標題畫面（owner 2026-08-01 17:05 增補）**：進站 `/` ＝遊戲標題畫面——ColorPlay icon＋「PRESS START」鈕；按下進既有登入頁（學生/教師身分切換沿用現行登入頁，不改）；登入後照現行固定導向進學習大廳/教師工作區。詳 §8。
8. **滿版舞台（owner 2026-08-01 21:59 裁定，取代第 1 項 letterbox）**：實機驗收後裁定置中 16:9 框「突兀」——舞台改**滿版貼齊視窗**（無框、無黑邊；`--stage-void` 保留為 token 但不再露出）；橫向模式保留「固定視窗＋HUD 釘住＋僅 scene 內容卷動」結構。§2 尺寸公式與舞台框語彙隨之作廢。
9. **減少上下捲動＝橫向/分頁方向（owner 2026-08-01 21:59 口頭，範圍待定）**：頁面內容盡量少用整頁上下卷動，改橫向或分頁顯示，避免整個畫面移動——屬內容層重排，各頁作法需另立 spec 與批次（候選：併批⑤b 或獨立「分頁批」）。
...
## 8. 標題畫面（owner 2026-08-01 17:05 增補）

- 路由不變：`/` 仍為既有路由，僅呈現層由通用 `RoutePage` 換成專屬 `TitlePage`；CTA 仍是 `Link` 到 `/login`。
- 內容：品牌三圓 icon（沿用既有 brand SVG 放大）＋「ColorPlay」像素字標題＋副標「色彩原理遊戲式學習平台」＋「PRESS START」鈕（≥44px、夜窗像素鈕、閃爍只動 opacity 且雙通道 reduced-motion 關閉、對比 ≥4.5:1 含閃爍最暗幀）。
- 背景：夜空（`--pixel-night-deep`），滿舞台。
- 身分選擇＝既有登入頁（學生帳號登入/教師端登入 切換已存在），零變更；`/unauthorized`、404 等其餘 `RoutePage` 用途不動。
- e2e 同步（顯式）：`app-shell.visual.spec.ts:14` 與 `create-app-router.test.tsx:179` 的 `前往登入` → `PRESS START`。
