...
| `admin_command_executions`／none | `created_at`,`completed_at`,`result_code` | `actor_principal_id`,`command_name`,`audit_event_id`,`request_id`,`redacted_result_receipt` | — | `idempotency_key`,`request_hash`,reveal plaintext |
| `admin_audit_principals`／none | `created_at`,`tombstoned_at` | `id` | `user_id`（mapping service only） | direct browser projection |
| `admin_audit_events`／audit screen | `occurred_at`,`action`,`target_type`,`result` | `id`,`actor_principal_id`,`admin_session_id`,`target_principal_id`,`request_id`,`correlation_id`,`reason_or_purpose_redacted`,`mfa_age_seconds`,`before_after_redacted`,`source_summary_redacted`,`compensates_event_id` | — | raw personal data、secret、revealed plaintext |
