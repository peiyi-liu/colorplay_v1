...
- `learning-experience.spec.ts` — 與 0730/0726 的 Live 改版**無關**（此檔不碰 Live）；紅因是另一條獨立斷差：`/join/:code` 路由已依 07-27/07-30 裁定整個移除，班級加入碼也從「一次性 receipt」改成「固定顯示在班級卡」，另加 1 處待補救題數標題已改成獨立 pill badge（DC 1063）不再嵌括號於可及名稱內。主體測驗/錯題/補救段落其餘文字對現行 UI **逐字存活**。**S–M，估 2 task。**
...
- Task B（S，視驗收結果可能升為 M）：**含 1 項標題可及名稱格式修正**——把 `:205` 的 `getByRole('heading',{name:/（2 題待補救）/u})` 改成比照 `mistakes-page.test.tsx:70-76` 現行寫法拆成兩段斷言（標題可及名稱 `{subtopicTitle} {count} 題待補救`，另外對 `.mistake-group__badge` 的 `getByText`+`toHaveClass` 斷言題數 pill）；此項選擇器改法明確、有現成測試可抄，不需探索，因此**不足以單獨把 S 上修到 M**——維持 S 判定，M 的風險仍主要來自下一步：實跑一次驗收模式，核對 reward/mistake 數字斷言與 `目前沒有待補救的錯題` 文案是否仍精確吻合；若吻合則此檔案結案，若有偏移則修正對應數字/文案。
...
**兩者皆非、是第三種：局部互動模型重寫（範圍很小）。** 主體（測驗作答、錯題、補救練習、教師學習進度視角）絕大部分是逐字對齊現行 UI 的**簽名比對修基線**級，改動量小——目前確認的簽名層級斷差只有 1 處（`:205` 待補救題數標題的可及名稱格式，DC 1063 拆成獨立 pill 後不再含全形括號）；但班級加入段（#1、#2）踩到的是「`/join/:code` 整條路由已被移除」——這是**互動模型層級**的斷裂（UI 入口消失，只能改走 RPC），只是**牽涉範圍只有這一小段**，不是整檔重寫。brief 提到的「錯題 33 開→約 25 開/8 解＋補救 XP」基線位移，本次唯讀盤點未能在本檔內定位到對應的 `student.one` 識別字或數字關聯，需要跑一次驗收模式才能確認是否真的影響本檔（見上方「待確認」項）。
