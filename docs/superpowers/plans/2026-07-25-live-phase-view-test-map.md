...
## Task 4：`live-presenter.test.tsx` 全部 6 case

| #           | case                                      | 去向                                                                                                                                         |
| ----------- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| PR1         | six-digit code and nickname wall          | 留（render）；`startLobbyLoop` 掛載即恢復＝Ambient Loop 屬性，斷言不變                                                                       |
| PR2         | regenerate hint                           | 留（render）                                                                                                                                 |
| PR3         | question with shaped options/ring/counter | 留（render）；暫停鈕改由 footerActions 清單提供                                                                                              |
| PR4         | reveals correct + Top 5                   | 留（render）；`playReveal` 掛載斷言**翻轉**為 not.toHaveBeenCalled（重連不發 Cue，plan Step 2 指定行為）→ 轉場發聲改由 cueFor 測試＋新增 PR7 |
| PR5         | podium reveal                             | 留（render）；`playFanfare` 同 PR4 翻轉                                                                                                      |
| PR6         | mute preference                           | 留（render）                                                                                                                                 |
| PR7（新增） | fires cues only on live phase transitions | rerender open→feedback→completed 逐一發聲一次（端到端可聽行為證明）                                                                          |

phase 選擇規則 → `projectorView` 測試；Cue 判定 → `cueFor` 測試（含重連 lobby 恢復、重連 podium 不發）。
