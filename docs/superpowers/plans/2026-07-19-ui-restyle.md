...
### Task 11b: 認證入口 GGAME 化（欄位仍 Email/密碼）— DONE

- [x] 主區深色放射漸層遮罩＋置中白卡（448px）＋「🎓 學生註冊登入／🧑‍🏫 教師診斷端」分段頁籤（選中黃／靛）＋教師權限提示框＋教師端靛色登入鈕。radio 語意保留（a11y/測試）。未選頁籤對比修至 AA（axe 綠）。e2e 47/47。
