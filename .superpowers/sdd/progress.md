...
Task 3: complete（strict release-record＋backup-manifest schemas/generators；原子 0600 寫入、SHA-256 sidecar、path/PII/secret fail-closed）。TDD RED 21/21 後，Phase 0 contracts 3 files/34 tests、lint、typecheck、Prettier 全綠；Vercel CLI 精確 pin `58.5.1`。僅完成 local evidence tooling，未進行 hosted mutation。詳見 `.superpowers/sdd/phase0-task-3-report.md`。
Task 4: complete（Feature CI 拆為 8 個 unique independent checks；protected `staging-approval` exact-SHA dispatch；`staging`/`main`/`prod-*` desired rulesets 無 bypass、禁 deletion/force-push）。TDD RED 8 項後 contracts 2 files/14 tests、lint、typecheck、Prettier 全綠；僅 repo desired state，未套用任何 hosted GitHub 設定。詳見 `.superpowers/sdd/phase0-task-4-report.md`。
Task 5: complete（repo-authoritative migration inventory＋fail-closed comparator；精確 aggregate-only Local collector，four drift classes，provider allowlist 預設空）。TDD RED 7/7 後 contracts 1 file/8 tests；乾淨 Local pgTAP 47 files/1070 tests、smoke 3 tests、integration 12 files/24 tests 全綠；真實 inventory self-compare `MIGRATION_DRIFT_ZERO`。未讀寫 hosted project，詳見 `.superpowers/sdd/phase0-task-5-report.md`。
Task 6: complete（age X25519 encrypted roles/schema/data/Storage/manifest；B2 writer-only create＋recovery-only verify；30-day Compliance lock、26h freshness、70/85/95 capacity與 overflow freeze）。TDD RED 5/5 後 backup/evidence contracts 2 files/26 tests、shellcheck 0.11.0、lint、typecheck、Prettier 全綠；fake-S3 僅 `.age`，未連線 B2/Hosted Supabase。詳見 `.superpowers/sdd/phase0-task-6-report.md`。
Task 7: complete（checksum-first/decrypt-second；unique temp Supabase project/ports；roles→schema→data→Storage；migration/row/Storage compare；bounded cleanup）。TDD RED 8/8 後 backup/restore contracts 2 files/13 tests、shellcheck、lint、typecheck、Prettier 全綠；計畫原命令真跑 synthetic drill 印出 `LOCAL_RESTORE_VERIFIED`，未觸碰 everyday Local/hosted target。詳見 `.superpowers/sdd/phase0-task-7-report.md`。
Task 8: complete（browser-backed read-only smoke；GET/HEAD-only＋Service Worker blocked；30-minute monitor；dedup incidents；3-consecutive web-only rollback）。TDD RED 8/8 後 contracts 1 file/9 tests、Chromium 2/2、shellcheck、lint、typecheck、Prettier 全綠；form POST 在 network 前被擋，security/data-corruption 只進 manual incident。未觸碰 hosted domain/Vercel。詳見 `.superpowers/sdd/phase0-task-8-report.md`。
Task 9: complete（舊 Staging bootstrap 永久 fail-closed；target/SHA/owner/backup/drift/mutation record 重驗；Auth/Storage 歸零後才建立 fixture；exact-SHA Staging deploy＋三瀏覽器/RWD＋real-device gate）。TDD RED 8/8 後 contracts 2 files/15 tests、shellcheck、lint、typecheck、Prettier 全綠；僅 repo desired state，未執行 hosted rebuild/deploy。詳見 `.superpowers/sdd/phase0-task-9-report.md`。
Task 10: complete（Production Candidate `--prod --skip-domain`；Candidate/Promotion 分離 Environment 與 credential；checksum-bound exact artifact promote；三次 read-only smoke 後 main/Vercel/tag SHA parity）。TDD RED 12/12 後 contracts 3 files/42 tests、lint、typecheck、Prettier、YAML parse 全綠；僅 repo desired state，未建立 Candidate、未 promote/推 main/tag。詳見 `.superpowers/sdd/phase0-task-10-report.md`。
Task 11: complete（ADR 0002 保留歷史並標示舊 Vercel item superseded；environment/readiness/Vercel/Staging runbooks 全面改為 Phase 0 two-slot＋exact-artifact 控制）。TDD RED 3/3 後 documentation/manifest contracts 2 files/7 tests、manifest write/check、Prettier 全綠；舊 direct-main/wipe/wildcard/credential/fixture-password guidance 零命中。詳見 `.superpowers/sdd/phase0-task-11-report.md`。
Task 12: `LOCAL IMPLEMENTATION READY — hosted configuration and gates NOT EXECUTED`（完整 local gate＋plan-base 重現＋嚴格 diff review）。install、lint、typecheck、build、DB 47 files/1070 tests＋smoke 1/3＋integration 12/24、Phase 0 contracts 11 files/96 tests、三環境 marker Chromium 各 1/1 全綠；unit coverage 131 files/916 tests 全過但既有全域門檻仍紅，format 14 檔與 Chromium 4 支紅燈亦在 `2295fd6` 基線精確重現，未降低任何 gate。OWNER GATE 0 的獨立 age-key 副本與 Staging/Production 分離 SMTP credential 仍 blocked；Task 13–18 hosted configuration/gates 均未執行。詳見 `.superpowers/sdd/phase0-task-12-report.md`。

Gate 1: complete（GitHub protective controls 已套用；PR #1 的 Foundation CI run `31091970044` 在 SHA `0d7d0e1fda6eee2842f0218377581f75cbc61663` 八項 required checks 全綠：format、lint、typecheck、unit coverage、Production build、Local database、Chromium E2E、credential scan）。PR 仍因 owner approval 規則為 `BLOCKED`，未合併至 `staging`；無 Staging 部署、DNS、Supabase data-plane mutation、Candidate、Production promotion 或 hosted backup/restore。B2 現有 writer/recovery credentials 權限過寬，真實 backup automation 仍停在 fail-closed gate，待 least-privilege rotation。

Gate 1 recovery: complete（`phase0-restore` 冷 GitHub runner 前一輪成功耗時 119.838 秒，碰上原 120 秒測試上限而在下一輪誤失敗；僅將 integrity drill 上限增為 300 秒，CI job 仍為 30 分鐘，RTO 目標仍為 8 小時）。focused isolated restore 8/8 全綠、無 `colorplay_restore_*` 容器殘留；PR SHA `5cb1ed35aec0ae8a99712a41143b5f6c7067e495` 的 Foundation CI run `31093226087` 八項 required checks 再次全綠。PR 仍 `BLOCKED` 待 owner approval，未合併或部署。

Final review: complete（範圍 `2295fd6..Task 12 closure`；逐 commit 隔離、90 檔 complete diff、workflow permissions/secret/target/rollback/evidence、spec §§1–17 與 stale runbook 全面核對）。已修復 rollback 隱性 success gate、Vercel token、HTTP redirect 假證據、六次監測／連三敗、real-device 人工證據、Environment 名稱、nested secret sanitization、Storage 全 bucket、真實 DB inventory restore、Candidate/Promotion exact-SHA supply-chain binding，以及 Local marker E2E false-green；終審無剩餘 Critical/Important。migrations、seeds、fixture credentials、login page、content import、Live product code 零變更；唯一產品變更為已核准的非秘密 Staging marker。Hosted provider、真實 Production backup/restore、DNS/TLS、real-device、Candidate、promotion、rollback 均未執行、不得宣稱已驗證。
Task 14: complete for the approved two-slot Candidate path（frozen source `9af07ee`；backup run `31158344282`＋restore run `31158754421` 全綠；prefix `production/2026/08/07/backup-20260807T074026Z/`，1,813,206 bytes、30-day COMPLIANCE 至 `2026-09-06T07:40:26Z`、30 日保守投影 54,396,217 bytes／9 GB = 0.6044%，restore 約 104 秒 < RTO 8h）。57 repo／57 Hosted ledger：48 exact＋9 同名 timestamp semantic drift、0 hosted-only、0 repo-only；comparator 依設計維持 blocked，禁止 in-place repair，Task 15 僅可在 clean Candidate migration-zero replay。Security Advisor 舊 Production 1 error／79 warn／3 info 已分類；forward migration `20260807000100` 以 invoker view＋safe-column grants＋owner-bound explanation helper 修 error，另固定 Live parser search path，未直接改 Production。DB 1080 pgTAP＋3 smoke＋24 integration、Phase 0 contracts 108、Vitest 964、lint/typecheck/Prettier/ShellCheck 全綠。Local restore partial-start network leak 已修；26 個舊 disposable networks 保留為未授權 bulk cleanup debt。詳見 `.superpowers/sdd/phase0-task-14-report.md`。
Task 14 correction: REOPENED／Task 15 BLOCKED（獨立全範圍審查確認舊 run `31158344282`／`31158754421` 未驗證 provider-read lifecycle、custom roles/Auth/authorization、app startup，且 restore workflow 曾在 job-wide recovery secrets 下 checkout caller-supplied SHA；因此上一行 complete 結論撤回但歷史證據保留）。`4a5044a` 已改為 protected `main|staging` trusted harness、manifest binding、step-scoped secrets、0600 restore logs＋sanitized sentinels、角色/Auth/RLS/ACL hash、authorization probe、application startup、Backblaze actual lifecycle 30+1 days 與 aggregate inventory disposition；Phase 0 contracts 11 files／111 tests、format、lint、typecheck、build 已綠。完整 coverage／restore 目前被 27 個已逐一確認 0 attached containers 的舊 `supabase_network_colorplay_restore_*` 耗盡 subnet 阻塞，待 owner 明確授權精確刪除；之後仍須 protected staging rerun corrected backup/restore，且 recovery key 需最小增加 `listBuckets`，不得先進 Task 15。

Task 14 local gate rerun + 二輪獨立 review（2026-08-09，commit `d274da0`，基準 `428dc78`）：延續上一行的 REOPENED 狀態，本輪把遺留的三個未提交檔案（`scripts/backup/restore-local.sh`、兩個 phase0 contracts test）走完整流程收斂。Docker 唯讀核實：先前「27 個已清除」之後又新累積 12 個孤立 `supabase_network_colorplay_restore_<pid>` network（逐一 inspect 確認 0 容器附著、無 active process），另有一組仍在健康運作的獨立 `colorplay_task14_9af07ee` replay stack（5 容器＋1 network，Up 2 天）；兩者皆與共享 `project_id=colorplay`（port 54321/54322）完全隔離，不阻塞 DB gate，但均未清理，不得再宣稱零殘留。DB gate 在確認無其他 session 佔用共享本機 Supabase 後執行：`pnpm test:db` PASS（48 pgTAP／1080 assertions、runtime smoke 3/3、integration 12/24、auth health 200、10/10 container healthy、secret scan 0 findings、exit 0），僅 db/auth/storage/realtime 重啟，共享 stack 其餘服務未受影響，未新增殘留。`database-inventory.json` invariant 追蹤：manifest 端為 optional（非 required），但 production backup 產生路徑無條件產出；restore 端 else 分支依賴 `public.synthetic_fixture` 表不存在而查詢失敗、`set -e` 中止，對真實 schema 是隱含 fail-closed，但無行為測試覆蓋此邊界（列為非阻塞 follow-up，不在本次修復範圍）。二輪 review 用 Codex CLI（`codex review`，單一 reviewer、一次往返）：發現 P2 finding——probe 被跳過時，report 仍把 `role_inventory`／`authorization_probe`／`application_startup` 寫死 `passed`，不誠實。修復：改依 `application_probe_required` 動態記錄 `passed`/`skipped`（TDD RED→GREEN）。Scoped verification：ShellCheck 乾淨、`phase0-backup`＋`phase0-restore` 兩檔重跑兩輪穩定 23/23（DB gate 後緊接的一次跑法出現一次不可複現的暫態失敗，判定為環境資源競爭非邏輯缺陷）、`phase0:contracts` 11/111、lint/typecheck/Prettier/`git diff --check` 全綠；Chromium gate（3/3）與完整 `pnpm vitest run`（135/967）於本輪修復前已驗證，未重跑（依「不需無理由重跑已通過 gate」指示）。逐檔 commit（禁 `-A`）為 `d274da0`。狀態邊界不變：即使 local gates 全綠且 remediation 已 commit，corrected hosted proof（owner-authorized push → protected CI/approval → merge staging → `listBuckets` credential rotation → fresh backup/restore from protected ref）仍未執行，Task 14 overall 仍非 complete，Task 15 仍不得開始。詳見 `.superpowers/sdd/phase0-task-14-report.md`「Local gate rerun and second review」一節。

Task 14A（獨立、小範圍 corrective task，非 Task 14 第二輪 review；2026-08-09，commit `a69f87b`，基準 `970f3df`）：移除 restore-local.sh 對「public.synthetic_fixture 剛好不存在」的隱含 fail-closed 依賴，改為 manifest 明確 discriminant。`create-manifest.mjs` 新增必要欄位 `artifact_kind: 'production' | 'synthetic_fixture'`（`create-manifest.d.mts` 同步）；`create-backup.sh` 兩條路徑分別寫入對應值——**此為刻意的 backward-incompatible manifest 變更**：此 commit 之前產生的舊 backup（含先前記錄的 `production/2026/08/07/backup-20260807T074026Z/`）缺少此欄位，之後會被 restore-local.sh 判為 `RESTORE_ARTIFACT_KIND_INVALID` 拒絕，需重新產生才能被還原；不動 `schema_version`（新增必要欄位，非既有欄位語義變更，且 restore-local.sh 本來就不透過 schema validator 讀取 manifest）。restore-local.sh 現在完全依 `artifact_kind` 分類：production 缺 `database-inventory.json` 在任何 schema/application probe 前即以 `RESTORE_DATABASE_INVENTORY_REQUIRED` 中止；缺失/未知/型別錯誤的 `artifact_kind` 一律 `RESTORE_ARTIFACT_KIND_INVALID`；checksum/signature/role-inventory/authorization-probe/application-startup 邏輯未動。TDD 矩陣（RED→GREEN）涵蓋 production+inventory 存在／缺失、artifact_kind 缺失/未知/型別錯誤，RED 階段意外發現舊邏輯不會快速拒絕、反而會啟動一個真實的隔離 Supabase stack（這正是本次要修的問題本身的證據）。單輪 review 用 `codex review`（單一 reviewer、一次往返）：發現 P1 finding 並以真實 repro 確認——bash command substitution 會剝除尾隨換行符,導致 `artifact_kind: "synthetic_fixture\n"` 被誤判為合法值繞過拒絕；malformed JSON 也會讓未捕捉的 Node 例外印出未消毒的 stack trace。修復：把驗證完全移入 Node 端（try/catch + 精確字串比對 + stderr 抑制），shell 端只看得到乾淨值或非零 exit code。修復後新增 trailing-newline/whitespace/malformed-JSON 測試案例並確認 GREEN，未再啟動第二輪 review。Scoped verification：ShellCheck、typecheck、scoped ESLint/Prettier、`git diff --check` 全綠；三個 contract test 檔案共 58/58、`phase0:contracts` 11 files/125 tests 全綠；未跑 `pnpm test:db`、`supabase db reset/start/stop`、完整 `pnpm vitest run`、`test:e2e`、`build` 或任何 Docker 清理，符合本 task 範圍限制。**意外副作用（已揭露、未清理）**：RED 階段幾個測試因 vitest 預設 5 秒 timeout 沒有 kill 底層 spawned bash 子進程，導致孤兒 `colorplay_restore_<pid>` supabase stack 在背景累積（一度同時有 18 個容器、5 組不同 stack），造成一次既有測試（未修改邏輯）的暫態資源競爭失敗，手動隔離重跑證實邏輯未壞；孤兒容器已自然清理完畢（commit 前確認 0 殘留）。既有的 12 個孤立 `supabase_network_colorplay_restore_*` network（Task 14 段落已記錄）維持不變，未清理、未新增。狀態邊界不變：本 task 只處理 local restore harness 的 artifact 分類正確性，不影響、不重新驗證、不取代已記錄的 hosted backup/restore 證據，也不清除「Remaining boundary」段落的 hosted-proof 要求。**Task 14 overall 仍非 complete，Task 15 仍不得開始。**詳見 `.superpowers/sdd/phase0-task-14-report.md`「Task 14A」一節。

Task 14A stop-time review follow-up（2026-08-09，commit `1c2ec39`）：session 自己的 Stop hook 在收工前自動觸發 Codex stop-time review，對 `a69f87b` diff 找出兩個 `a69f87b` 沒修到的 P1 契約缺陷：(1) `restore-local.sh:80` 既有的 `RESTORE_EXPECTED_REPO_SHA` 解析邏輯緊接在新版 `artifact_kind` sanitizer**之前**，仍用未受保護的 `node -e` 呼叫；protected workflow 一定會設定這個環境變數，malformed manifest 在這裡會洩漏未消毒的 Node stack trace/路徑——正是 `a69f87b` 才修過的同一類問題,只是早一步；`a69f87b` 的回歸測試沒設這個環境變數所以沒測到。(2) `docs/deployment/backup-manifest.schema.json` 是一份獨立、`additionalProperties: false` 的正式 manifest schema，程式碼完全沒有自動化強制力驗證,`a69f87b` 新增 `artifact_kind` 完全沒同步到這份文件，導致所有新 manifest 違反正式 schema；先前沒有任何測試引用這份檔案，漂移完全沒有訊號。修復：把 `repo_sha` 與 `artifact_kind` 檢查合併成一次 try/catch＋stderr 抑制的 Node 呼叫（消除未受保護的解析,而非重複相同 sanitizer pattern）；schema 補上 `artifact_kind`，並新增 schema-sync contract test（比對 schema 宣告欄位與 `create-manifest.mjs` 實際輸出欄位,未來再漂移會立刻測試失敗)。TDD RED→GREEN。Verification：ShellCheck/typecheck/scoped ESLint/Prettier/`git diff --check` 全綠；三個 contract test 檔案共 60/60；`phase0:contracts` 11/127；Docker 0 殘留容器、12 個既有殘留 network 不變、共享 stack 未受影響。這是同一輪 Stop-time review 的 finding,直接修復驗證,未啟動第二輪 Codex review。狀態邊界不變：**Task 14 overall 仍非 complete，Task 15 仍不得開始。**詳見 `.superpowers/sdd/phase0-task-14-report.md`「Stop-time review follow-up」一節。

Task 14A 第二輪 stop-time review follow-up（2026-08-10，commit `b0471dd`）：第二次自動 Stop-time review 對 `1c2ec39` diff 找出還是不完整——`1c2ec39` 只消毒了 `restore-local.sh` 的解析,但實際的 protected `.github/workflows/restore-drill.yml` 在呼叫 `restore-local.sh` **之前**、"Verify decrypted manifest binding" 這個 step 裡有自己**獨立**的、未受保護的 `JSON.parse`（inline `node --input-type=module -` heredoc）,malformed manifest 到這裡就會洩漏 Node stack trace 與 `$RUNNER_TEMP` 路徑——同一類問題在第三個獨立程式碼路徑又出現一次。`1c2ec39` 那個測試名稱聲稱涵蓋"protected workflow"，但其實只是模擬環境變數直接呼叫 `restore-local.sh`，從未真正執行過 workflow 自己的這段解析,給了假的信心。修復：把這段 inline heredoc 抽成獨立腳本 `scripts/backup/verify-manifest-binding.mjs`（遵循既有的 `create-manifest.mjs` 等 pattern：頂層 try/catch,所有失敗模式統一收斂成一個消毒過的 `RESTORE_MANIFEST_BINDING_INVALID`），workflow 改呼叫這個腳本。新增測試直接 spawn 這個腳本驗證各種失敗情境；為了確認測試真的有效而非空洞，暫時把安全實作換回原始未受保護版本重跑，真實重現了洩漏完整 `file://` 路徑與 stack trace 的問題,再換回安全版本確認 GREEN。Verification 全綠（typecheck/scoped ESLint/Prettier/`git diff --check`；`phase0-restore.test.ts` 32/32；`phase0:contracts` 11/133；Docker 0 殘留容器、既有 12 個殘留 network 不變、共享 stack 未受影響）。同一輪 Stop-time review 週期,未啟動新的 Codex review round。狀態邊界不變：**Task 14 overall 仍非 complete，Task 15 仍不得開始。**詳見 `.superpowers/sdd/phase0-task-14-report.md`「Second stop-time review follow-up」一節。
