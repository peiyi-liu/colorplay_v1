...
ColorPlay 是瀏覽器執行的遊戲式教學平台（第一領域：技術型高中設計群－色彩原理），從單一 HTML 原型重構為 React + TypeScript + Supabase。目標涵蓋複習／Quiz／補救／進度、XP／Token／Blook／成就、班級／Assignments／排行榜、第一方 ColorPlay Live、教師內容／分析與研究匯出。所有正式答案、成績、獎勵、進度、排名與權限由後端決定。
...
spec 對照表：架構/API/Realtime → `02`；資料表/RLS → `03`；登入/安全 → `04`；分數/XP/進度/成就/Live rules → `05`；題庫 → `06`；UI/RWD → `07`；測試 → `08`；效能 → `09`；phase ownership → `10`。環境與遷移決策查 `docs/{adr,deployment,migration}/`。
...
React + TypeScript + Vite、Tailwind（CSS variables tokens）、React Router、TanStack Query、RHF + Zod、Zustand（僅 client ephemeral）、Supabase（Auth/PostgreSQL/RLS/Storage/Realtime/RPC/Edge Functions）、Vitest + RTL、Playwright、pnpm。無核准 ADR 不得更換；不得引入 Angular/Vue/Next.js/Firebase 或第二套資料庫。
...
以 feature 切分（`src/features/{auth,profile,learning,quiz,remediation,progress,rewards,achievements,inventory,classrooms,assignments,leaderboard,live,teacher}`），共用放 `components/`、`lib/`、`styles/`、`types/`；DB 相關放 `supabase/{migrations,functions,seed.sql,tests}`；測試放 `tests/{contracts,integration,e2e,visual,fixtures,acceptance}`；治理文件放 `docs/{adr,deployment,migration,superpowers}`；證據放 `artifacts/acceptance/`（不進 git）。單檔超過 500 行需說明理由或拆分。
...
`dev / build / preview / lint / typecheck / test / test:coverage / test:db / test:e2e / test:visual / acceptance`。現有 `pnpm acceptance`／Foundation Task 16 依 ADR 0001 延後至 Phase 8 release gate；日常 task 與中間 phase 不執行。未來 phase 若需要里程碑 evidence，必須由該 phase 已核准 plan 指定 scoped command，不能偷用既有完整 gate 宣稱 Production-ready。
...
**每個已核准 phase gate（里程碑，一個 phase 只做一次）**：依該 phase plan 跑完整受影響測試、真實 Supabase local/staging、必要 browser/viewports、console/network、證據 manifest 與一次 review。現有全域 `pnpm acceptance`、GitHub/Vercel wiring、Production smoke、完整三 browser/headed/real-device release proof 統一留到 Phase 8。真實行動裝置證據（AC-UI-010/012）由**人類**提供，代理不得模擬或反覆重試，只標記「待人工裝置驗證」。
