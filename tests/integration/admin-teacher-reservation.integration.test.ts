...
describe('teacher login reservation under real concurrency', () => {
  ...
  let fixtureProfileSuffix = 97;
  let fixtureOperationSuffix = 98;
  let firstReservedSuffix = 99;
  ...
  beforeAll(() => {
    ...
    const boundaryPower = Math.max(
      100,
      10 ** String(occupiedMaximum + 2).length,
    );
    fixtureProfileSuffix = boundaryPower - 2;
    fixtureOperationSuffix = boundaryPower - 1;
    firstReservedSuffix = boundaryPower;
    if (firstReservedSuffix + REQUEST_COUNT - 1 > 9_999_999_999_999) {
      throw new Error(
    ...
    runSql(`
      update public.profiles
         set role = 'teacher', login_account = 'teacher${String(fixtureProfileSuffix)}',
             full_name = '既有教師 ${String(fixtureProfileSuffix)}',
             display_name = '既有教師 ${String(fixtureProfileSuffix)}'
      ...
      insert into admin_private.teacher_account_operations (
        ...
        'teacher${String(fixtureOperationSuffix)}',
        '既有保留教師 ${String(fixtureOperationSuffix)}', '${fixtureTag}-floor'
      );
    `);

    // Start far behind the occupied high-water mark. Concurrent results must
    // begin at the next decimal-width boundary without truncating digits.
    runSql(
  ...
  it('reserves unique operation IDs, suffixes and login accounts', async () => {
    ...
    expect(suffixes).toEqual(
      Array.from(
        ...
        (_, index) => firstReservedSuffix + index,
      ),
    );
    expect(parsed.map((row) => row.loginAccount)).toContain(
      `teacher${String(firstReservedSuffix)}`,
    );
    expect(parsed.map((row) => row.loginAccount)).toContain(
      `teacher${String(firstReservedSuffix + 1)}`,
    );
