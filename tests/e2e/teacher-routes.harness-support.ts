...
export const verifyDrillDownComposition = async (
  ...
  if (scenario === 'student-progress' && width === 393) {
    const summary = page
      .getByTestId('chapter-disclosure')
      .first()
      .locator('summary');
    await expect(summary).toHaveAttribute('aria-expanded', 'false');
    await summary.click();
  ...
  if (scenario === 'student-progress' && width === 1280)
    await expect(
      page.getByRole('table', { name: '各章節學習進度' }),
    ).toBeVisible();
};
...
export const verifyLiveReportComposition = async (
  ...
  if (
    ...
    (width !== 320 && width !== 393 && width !== 1280)
  )
    return;
  const matrix = page.getByRole('region', { name: '作答矩陣' });
  await expect(matrix).toBeVisible();
  const matrixBounds = await matrix
    .locator('.live-matrix-scroll')
    .evaluate((el) => ({
      clientWidth: el.clientWidth,
      scrollWidth: el.scrollWidth,
    }));
  expect(matrixBounds.clientWidth).toBeLessThanOrEqual(
    matrixBounds.scrollWidth,
  );
  if (width === 393) {
    ...
    expect(new Set(ranks.map((entry) => Math.round(entry.bottom))).size).toBe(
      1,
    );
  }
};
