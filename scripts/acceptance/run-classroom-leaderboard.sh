...
if test -n "$git_status"; then
  printf 'CLASSROOM_LEADERBOARD_DIRTY_WORKTREE\n' >&2
  exit 1
fi

git_sha="$(git rev-parse HEAD)"
phase_root="$project_root/artifacts/acceptance/classroom-leaderboard-${git_sha}"
if test -e "$phase_root"; then
  printf 'CLASSROOM_LEADERBOARD_EVIDENCE_ALREADY_EXISTS\n' >&2
  exit 1
fi
...
node -e '
  ...
  writeFileSync(path, `${JSON.stringify({
    acceptance_ids: [
      "AC-AUTH-005", "AC-AUTH-006", "AC-AUTH-007",
      "AC-GAME-008", "AC-GAME-009"
    ],
...
if test "$(git rev-parse HEAD)" != "$git_sha" || \
  ...
  printf 'CLASSROOM_LEADERBOARD_SOURCE_STATE_CHANGED\n' >&2
  exit 1
fi

node scripts/acceptance/finalize-classroom-leaderboard.mjs "$phase_root"
