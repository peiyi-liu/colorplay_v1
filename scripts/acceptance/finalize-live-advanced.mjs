...
export const ACCEPTANCE_IDS = Object.freeze(['AC-LIVE-012']);

const ANSWER_P95_BUDGET_MS = 800;
const FINALIZE_P95_BUDGET_MS = 1000;
const MINIMUM_ANSWER_SAMPLES = 30;
...
const COMMAND_LABELS = Object.freeze([
  ...
  "bash scripts/test-e2e-local.sh --project=chromium --headed --grep='Live Advanced phase gate'",
]);
...
const EXPECTED_BROWSER_FAILURES = Object.freeze([
  Object.freeze({
    ...
    observed_count: 1,
    status: 400,
    url_pattern: /\/rest\/v1\/rpc\/join_live_session(?:\?.*)?$/u.source,
  }),
]);
...
const translateEvidenceError = (error) => {
  ...
  const messages = {
    EVIDENCE_INVALID_BINARY: 'LIVE_ADVANCED_INVALID_BINARY_EVIDENCE',
    EVIDENCE_INVALID_PATH: 'LIVE_ADVANCED_INVALID_EVIDENCE_PATH',
    EVIDENCE_REQUIRED_MISSING: 'LIVE_ADVANCED_REQUIRED_EVIDENCE_MISSING',
    EVIDENCE_SENSITIVE: 'LIVE_ADVANCED_SENSITIVE_EVIDENCE',
  };
...
const relativeEvidencePath = (root, path) => {
  ...
  if (!output || output.startsWith('../') || output.includes('/../')) {
    throw new Error('LIVE_ADVANCED_INVALID_EVIDENCE_PATH');
  }
  return output;
};
...
const parseCommands = async (root) => {
  ...
  if (rows.length !== COMMAND_LABELS.length) {
    throw new Error('LIVE_ADVANCED_REQUIRED_EVIDENCE_MISSING');
  }
  const commands = [];
  for (const [index, row] of rows.entries()) {
    ...
    if (
      ...
      throw new Error('LIVE_ADVANCED_COMMAND_REPORT_INVALID');
    }
...
const assertSourceState = (run) => {
  if (
    ...
    throw new Error('LIVE_ADVANCED_INVALID_SOURCE_STATE');
  }
};
const assertBrowserHealth = (health) => {
  if (
    ...
    throw new Error('LIVE_ADVANCED_BROWSER_HEALTH_FAILED');
  }
};

// AC-LIVE-012: the latency profile is a measured artifact, and the gate
// fails closed unless every budget holds.
const assertLatencyProfile = (profile) => {
  if (
    !isPlainObject(profile) ||
    !isPlainObject(profile.answer) ||
    !isPlainObject(profile.finalize) ||
    !Array.isArray(profile.cold_start_ms) ||
    profile.cold_start_ms.length < 2 ||
    profile.answer.count < MINIMUM_ANSWER_SAMPLES ||
    typeof profile.answer.p95_ms !== 'number' ||
    profile.answer.p95_ms > ANSWER_P95_BUDGET_MS ||
    profile.finalize.count < 2 ||
    typeof profile.finalize.p95_ms !== 'number' ||
    profile.finalize.p95_ms > FINALIZE_P95_BUDGET_MS ||
    profile.lost_answers !== 0 ||
    profile.duplicate_answers !== 0 ||
    profile.outsider_participant_rows !== 0
  ) {
    throw new Error('LIVE_ADVANCED_LATENCY_PROFILE_FAILED');
  }
};

export async function finalizeLiveAdvanced(runDirectory) {
  ...
  const requiredScreenshots = [
    'live-host-1440x900.png',
    'live-team-768x1024.png',
    'live-report-375x812.png',
  ];
  if (
  ...
  const latencyProfile = await readJson(
    join(root, 'reports/latency-profile.json'),
  );
  assertLatencyProfile(latencyProfile);
  await assertSafe({
    evidencePaths: [
      join(root, 'run.json'),
      ...screenshots,
...
if (invokedPath === import.meta.url) {
  ...
  if (!runDirectory) {
    process.stderr.write('LIVE_ADVANCED_FINALIZER_ARGUMENT_REQUIRED\n');
    process.exitCode = 1;
  } else {
    try {
      await finalizeLiveAdvanced(runDirectory);
    } catch (error) {
      process.stderr.write(
        `${
          error instanceof Error
            ? error.message
            : 'LIVE_ADVANCED_FINALIZER_FAILED'
        }\n`,
      );
      process.exitCode = 1;
    }
