...
const COMMAND_LABELS = Object.freeze([
  ...
  "bash scripts/test-e2e-local.sh --project=chromium --headed --grep='Learning Experience phase gate'",
]);
...
const translateEvidenceError = (error) => {
  ...
  const messages = {
    EVIDENCE_INVALID_BINARY: 'LEARNING_EXPERIENCE_INVALID_BINARY_EVIDENCE',
    EVIDENCE_INVALID_PATH: 'LEARNING_EXPERIENCE_INVALID_EVIDENCE_PATH',
    EVIDENCE_REQUIRED_MISSING: 'LEARNING_EXPERIENCE_REQUIRED_EVIDENCE_MISSING',
    EVIDENCE_SENSITIVE: 'LEARNING_EXPERIENCE_SENSITIVE_EVIDENCE',
  };
...
const relativeEvidencePath = (root, path) => {
  ...
  if (!output || output.startsWith('../') || output.includes('/../')) {
    throw new Error('LEARNING_EXPERIENCE_INVALID_EVIDENCE_PATH');
  }
  return output;
};
...
const parseCommands = async (root) => {
  ...
  if (rows.length !== COMMAND_LABELS.length) {
    throw new Error('LEARNING_EXPERIENCE_REQUIRED_EVIDENCE_MISSING');
  }
  const commands = [];
  for (const [index, row] of rows.entries()) {
    ...
    if (
      ...
      throw new Error('LEARNING_EXPERIENCE_COMMAND_REPORT_INVALID');
    }
...
const assertSourceState = (run) => {
  if (
    !isPlainObject(run) ||
    run.phase !== 'learning-experience-v1' ||
    ...
    throw new Error('LEARNING_EXPERIENCE_INVALID_SOURCE_STATE');
  }
};
const assertBrowserHealth = (health) => {
  if (
    ...
    throw new Error('LEARNING_EXPERIENCE_BROWSER_HEALTH_FAILED');
  }
};

export async function finalizeLearningExperience(runDirectory) {
...
if (invokedPath === import.meta.url) {
  ...
  if (!runDirectory) {
    process.stderr.write('LEARNING_EXPERIENCE_FINALIZER_ARGUMENT_REQUIRED\n');
    process.exitCode = 1;
  } else {
    try {
      await finalizeLearningExperience(runDirectory);
    } catch (error) {
      process.stderr.write(
        `${
          error instanceof Error
            ? error.message
            : 'LEARNING_EXPERIENCE_FINALIZER_FAILED'
        }\n`,
      );
      process.exitCode = 1;
    }
