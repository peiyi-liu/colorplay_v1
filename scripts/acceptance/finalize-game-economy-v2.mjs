...
const scanTraceArchive = async (tracePath) => {
  ...
  try {
    ...
    for (const path of await listFiles(temporary)) {
      if (containsSensitiveValue(await readFile(path))) return true;
    }
    return false;
  } finally {
...
const assertSensitiveEvidenceAbsent = async (root, paths, tracePaths) => {
  ...
  for (const path of paths) {
    const sensitive = traceSet.has(path)
      ? await scanTraceArchive(path)
      : containsSensitiveValue(await readFile(path));
