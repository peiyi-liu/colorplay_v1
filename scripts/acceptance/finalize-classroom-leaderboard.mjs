...
const translateEvidenceError = (error) => {
  ...
  const messages = {
    EVIDENCE_INVALID_BINARY: 'CLASSROOM_LEADERBOARD_INVALID_BINARY_EVIDENCE',
    EVIDENCE_INVALID_PATH: 'CLASSROOM_LEADERBOARD_INVALID_EVIDENCE_PATH',
    EVIDENCE_REQUIRED_MISSING:
      'CLASSROOM_LEADERBOARD_REQUIRED_EVIDENCE_MISSING',
    EVIDENCE_SENSITIVE: 'CLASSROOM_LEADERBOARD_SENSITIVE_EVIDENCE',
  };
...
const relativeEvidencePath = (root, path) => {
  ...
  if (!output || output.startsWith('../') || output.includes('/../')) {
    throw new Error('CLASSROOM_LEADERBOARD_INVALID_EVIDENCE_PATH');
  }
  return output;
};
...
const parseCommands = async (root) => {
  ...
  if (rows.length !== COMMAND_LABELS.length) {
    throw new Error('CLASSROOM_LEADERBOARD_REQUIRED_EVIDENCE_MISSING');
  }
  const commands = [];
  for (const [index, row] of rows.entries()) {
    ...
    if (
      ...
      throw new Error('CLASSROOM_LEADERBOARD_COMMAND_REPORT_INVALID');
    }
...
const assertSourceState = (run) => {
  if (
    !isPlainObject(run) ||
    run.phase !== 'classroom-leaderboard-v2' ||
    ...
    throw new Error('CLASSROOM_LEADERBOARD_INVALID_SOURCE_STATE');
  }
};
...
export async function finalizeClassroomLeaderboard(runDirectory) {
...
if (invokedPath === import.meta.url) {
  ...
  if (!runDirectory) {
    process.stderr.write('CLASSROOM_LEADERBOARD_FINALIZER_ARGUMENT_REQUIRED\n');
    process.exitCode = 1;
  } else {
    try {
      await finalizeClassroomLeaderboard(runDirectory);
    } catch (error) {
      process.stderr.write(
        `${
          error instanceof Error
            ? error.message
            : 'CLASSROOM_LEADERBOARD_FINALIZER_FAILED'
        }\n`,
      );
      process.exitCode = 1;
    }
