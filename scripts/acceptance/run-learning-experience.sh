...
if test -n "$git_status"; then
  printf 'LEARNING_EXPERIENCE_DIRTY_WORKTREE\n' >&2
  exit 1
fi

git_sha="$(git rev-parse HEAD)"
phase_root="$project_root/artifacts/acceptance/learning-experience-${git_sha}"
if test -e "$phase_root"; then
  printf 'LEARNING_EXPERIENCE_EVIDENCE_ALREADY_EXISTS\n' >&2
  exit 1
fi
...
node -e '
  ...
  writeFileSync(path, `${JSON.stringify({
    acceptance_ids: [
      "AC-LEARN-001", "AC-LEARN-002", "AC-LEARN-003", "AC-LEARN-004",
      "AC-PROG-001", "AC-PROG-002", "AC-PROG-003", "AC-PROG-004",
      "AC-PROG-005", "AC-PROG-006"
    ],
...
run_logged \
  ...
  bash scripts/test-e2e-local.sh \
    --project=chromium \
    --headed \
    --grep='Learning Experience phase gate'
...
while IFS= read -r screenshot; do
  ...
  find "$phase_root/playwright" \
    -type f \
    \( -name 'chapter-detail-*.png' \
      -o -name 'review-card-*.png' \
      -o -name 'progress-dashboard-*.png' \) \
    -print
)
...
if test "$(git rev-parse HEAD)" != "$git_sha" || \
  ...
  printf 'LEARNING_EXPERIENCE_SOURCE_STATE_CHANGED\n' >&2
  exit 1
fi

node scripts/acceptance/finalize-learning-experience.mjs "$phase_root"
