...
const EXPECTED_BROWSER_FAILURES = Object.freeze([
  ...
  Object.freeze({
    ...
    observed_count: 1,
    status: 400,
    url_pattern: /\/rest\/v1\/rpc\/advance_live_session(?:\?.*)?$/u.source,
  }),
]);
...
const translateEvidenceError = (error) => {
  ...
  const messages = {
    EVIDENCE_INVALID_BINARY: 'ASSIGNMENTS_LIVE_INVALID_BINARY_EVIDENCE',
    EVIDENCE_INVALID_PATH: 'ASSIGNMENTS_LIVE_INVALID_EVIDENCE_PATH',
    EVIDENCE_REQUIRED_MISSING: 'ASSIGNMENTS_LIVE_REQUIRED_EVIDENCE_MISSING',
    EVIDENCE_SENSITIVE: 'ASSIGNMENTS_LIVE_SENSITIVE_EVIDENCE',
  };
...
const relativeEvidencePath = (root, path) => {
  ...
  if (!output || output.startsWith('../') || output.includes('/../')) {
    throw new Error('ASSIGNMENTS_LIVE_INVALID_EVIDENCE_PATH');
  }
  return output;
};
...
const parseCommands = async (root) => {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (
      ...
      throw new Error('ASSIGNMENTS_LIVE_COMMAND_REPORT_INVALID');
    }
...
const assertSourceState = (run) => {
  if (
    ...
    throw new Error('ASSIGNMENTS_LIVE_INVALID_SOURCE_STATE');
  }
};
const assertBrowserHealth = (health) => {
  if (
    ...
    throw new Error('ASSIGNMENTS_LIVE_BROWSER_HEALTH_FAILED');
  }
};

export async function finalizeAssignmentsLive(runDirectory) {
  ...
  const liveLatency = await readJson(join(root, 'reports/live-latency.json'));
  if (
    !Number.isInteger(liveLatency?.answer_samples) ||
    liveLatency.answer_samples < 30 ||
    typeof liveLatency.answer_p95_ms !== 'number' ||
    liveLatency.answer_p95_ms > 800 ||
    typeof liveLatency.finalize_p95_ms !== 'number' ||
    liveLatency.finalize_p95_ms > 1000 ||
    liveLatency.lost_or_duplicate_answers !== 0 ||
    liveLatency.outsider_access !== 0
  ) {
    throw new Error('ASSIGNMENTS_LIVE_LATENCY_GATE_FAILED');
  }
  await assertSafe({
    evidencePaths: [
      join(root, 'run.json'),
...
if (invokedPath === import.meta.url) {
  ...
  if (!runDirectory) {
    process.stderr.write('ASSIGNMENTS_LIVE_FINALIZER_ARGUMENT_REQUIRED\n');
    process.exitCode = 1;
  } else {
    try {
      await finalizeAssignmentsLive(runDirectory);
    } catch (error) {
      process.stderr.write(
        `${
          error instanceof Error
            ? error.message
            : 'ASSIGNMENTS_LIVE_FINALIZER_FAILED'
        }\n`,
      );
      process.exitCode = 1;
    }
