...
if test -n "$git_status"; then
  printf 'LIVE_ADVANCED_DIRTY_WORKTREE\n' >&2
  exit 1
fi

git_sha="$(git rev-parse HEAD)"
phase_root="$project_root/artifacts/acceptance/live-advanced-${git_sha}"
if test -e "$phase_root"; then
  printf 'LIVE_ADVANCED_EVIDENCE_ALREADY_EXISTS\n' >&2
  exit 1
fi
...
while IFS= read -r screenshot; do
  ...
  find "$phase_root/playwright" \
    -type f \
    \( -name 'live-host-*.png' \
      -o -name 'live-team-*.png' \
      -o -name 'live-report-*.png' \) \
    -print
)
...
if test "$(git rev-parse HEAD)" != "$git_sha" || \
  ...
  printf 'LIVE_ADVANCED_SOURCE_STATE_CHANGED\n' >&2
  exit 1
fi

node scripts/acceptance/finalize-live-advanced.mjs "$phase_root"
