...
if test -n "$git_status"; then
  printf 'GAME_ECONOMY_DIRTY_WORKTREE\n' >&2
  exit 1
fi

git_sha="$(git rev-parse HEAD)"
phase_root="$project_root/artifacts/acceptance/game-economy-v2-${git_sha}"
if test -e "$phase_root"; then
  printf 'GAME_ECONOMY_EVIDENCE_ALREADY_EXISTS\n' >&2
  exit 1
fi
...
node -e '
  ...
  writeFileSync(path, `${JSON.stringify({
    acceptance_ids: [
      "AC-GAME-001", "AC-GAME-002", "AC-GAME-003",
      "AC-GAME-004", "AC-GAME-005", "AC-GAME-006",
      "AC-GAME-007", "AC-SEC-001", "AC-SEC-002"
    ],
...
run_logged \
  'pnpm lint' \
  "$phase_root/reports/lint.log" \
  pnpm lint
run_logged \
  'pnpm typecheck' \
  "$phase_root/reports/typecheck.log" \
  pnpm typecheck
run_logged \
  'pnpm test' \
  "$phase_root/reports/unit.log" \
  pnpm test
run_logged \
  'pnpm build' \
  "$phase_root/reports/build.log" \
  pnpm build
run_logged \
  'pnpm test:db' \
  "$phase_root/reports/database.log" \
  pnpm test:db
...
if test "$(git rev-parse HEAD)" != "$git_sha" || \
  ...
  printf 'GAME_ECONOMY_SOURCE_STATE_CHANGED\n' >&2
  exit 1
fi

node scripts/acceptance/finalize-game-economy-v2.mjs "$phase_root"
